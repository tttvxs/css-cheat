...
void dump_client_class(const char* network_name, RecvTable* table, uint32_t offset) {
	for (int i = 0; i < table->m_nProps; i++) {
		...
		if (prop->m_RecvType == DPT_DataTable && prop->GetDataTable() && prop->GetDataTable()->GetName()[0] == 'D') {
			dump_client_class(network_name, prop->GetDataTable(), offset + prop->GetOffset());
		}
		dumped_classes.push_back(network_name);
		dumped_netvars.push_back({ prop->GetType(), network_name, prop->m_pVarName, prop });
		//dump << type2text(prop->GetType()) << " | " << network_name << " -> " << prop->m_pVarName << " : 0x" << std::hex << offset + prop->GetOffset() << std::dec << std::endl;
	}
}

void netvars_dump() {
	dump.open("C:\\Users\\gfhhfghjfgh\\Desktop\\css\\css\\x64\\Debug\\dump.txt");

	for (ClientClass* client_class = chlclient->GetAllClasses(); client_class; client_class = client_class->m_pNext) {
